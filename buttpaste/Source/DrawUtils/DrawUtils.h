...
void FovCutsom(float r, bool filled, bool rainbow, bool toMouse, float rainbowSpeed)
{
    ...
    for (int i = 0; i < sides; ++i)
    {
        ...
        float angleNext = angle + 2 * M_PI / sides;
        ImVec2 lastPos(center.x + cos(angle)     * r, center.y + sin(angle)     * r);
        ImVec2 nextPos(center.x + cos(angleNext) * r, center.y + sin(angleNext) * r);
        drawList->AddLine(lastPos, nextPos, IM_COL32(0, 0, 0, 255), 3.0f);
    }
    ...
    for (int i = 0; i < sides; ++i)
    {
        ...
        ImVec2 lastPos(center.x + cos(angle)     * r, center.y + sin(angle)     * r);
        ImVec2 nextPos(center.x + cos(angleNext) * r, center.y + sin(angleNext) * r);
        drawList->AddLine(lastPos, nextPos, IM_COL32(255, 255, 255, 255), 1.5f);
    }
}
